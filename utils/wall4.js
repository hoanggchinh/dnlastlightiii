...
const diskFragmentShader = `
    ...
    // Simplex Noise functions (giữ nguyên)
...
export function startBlackHole() {
    ...
    renderer.domElement.style.pointerEvents = 'none'; // Quan trọng: cho phép click xuyên qua canvas
    ...
    // 3. Controls (Cho phép quay tự động)
    controls = new OrbitControls(camera, renderer.domElement);
    controls.enableDamping = true;
    controls.dampingFactor = 0.035;
    controls.rotateSpeed = 0.4;
    controls.autoRotate = true; // Bật Auto Rotate
    ...
    const starMaterial = new THREE.ShaderMaterial({
        ...
        fragmentShader: `
            ...
            void main() { float dist = distance(gl_PointCoord, vec2(0.5)); if (dist > 0.5) discard; float alpha = 1.0 - smoothstep(0.0, 0.5, dist); alpha *= (0.2 + vTwinkle * 0.8); gl_FragColor = vec4(vColor, alpha); }
        `,
...
// Hàm dừng và dọn dẹp Live Wallpaper
export function stopBlackHole() {
    ...
    // Dọn dẹp bộ nhớ Three.js
    if(scene) {
        scene.traverse(object => {
            if (object.geometry) object.geometry.dispose();
            if (object.material) {
                // Kiểm tra nếu là ShaderMaterial để gọi dispose cho các uniforms
    ...
    // Xóa Renderer và canvas khỏi DOM
    if(renderer) {
        renderer.dispose();
        const container = document.getElementById('live-wallpaper-container');
        if (container) {
